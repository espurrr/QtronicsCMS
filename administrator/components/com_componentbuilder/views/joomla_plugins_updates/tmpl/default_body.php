...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if ($canDo->get('joomla_plugin_updates.edit')): ?>
		...
		<td class="nowrap">
			<div class="name">
				<?php if ($canDo->get('joomla_plugin_updates.edit')): ?>
					...
					<?php if ($item->checked_out): ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $userChkOut->name, $item->checked_out_time, 'joomla_plugins_updates.', $canCheckin); ?>
					<?php endif; ?>
				<?php else: ?>
		...
		<?php if ($canDo->get('joomla_plugin_updates.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo JHtml::_('jgrid.published', $item->published, $i, 'joomla_plugins_updates.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo JHtml::_('jgrid.published', $item->published, $i, 'joomla_plugins_updates.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo JHtml::_('jgrid.published', $item->published, $i, 'joomla_plugins_updates.', true, 'cb'); ?>
				<?php endif; ?>
