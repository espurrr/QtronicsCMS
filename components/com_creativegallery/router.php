 ...
 */
function CreativegalleryBuildRoute( &$query )
{
	static $items;
	...
	if (!$items) {
		$component	= JComponentHelper::getComponent('com_creativegallery');
		$menu	= $app->getMenu();
	...
	if (is_array($items))
	{
		...
		if (!$itemid) {
			foreach ($items as $item)
			{
				...
				if (isset($item->query['view']) && $item->query['view'] == 'Creativegallery'
		...
		if (!$itemid) {
			foreach ($items as $item)
			{
				if (isset($query['view']) && $query['view'] == 'creativegallery' && isset($item->query['view']) && $item->query['view'] == 'creativegallery')
				{
 ...
 * @return	array
 */
function CreativegalleryParseRoute( $segments )
{
	$vars = array();
