...
class ComponentbuilderViewPlaceholder extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_COMPONENTBUILDER_PLACEHOLDER_NEW' : 'COM_COMPONENTBUILDER_PLACEHOLDER_EDIT'), 'pencil-2 article-add');
		...
		if (ComponentbuilderHelper::checkString($this->referral))
		{
			if ($this->canDo->get('placeholder.create') && $isNew)
			{
				...
				JToolBarHelper::save('placeholder.save', 'JTOOLBAR_SAVE');
			}
			...
			{
				// We can close it.
				JToolBarHelper::cancel('placeholder.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				{
					JToolBarHelper::apply('placeholder.apply', 'JTOOLBAR_APPLY');
					JToolBarHelper::save('placeholder.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('placeholder.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('placeholder.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				{
					JToolbarHelper::versions('com_componentbuilder.placeholder', $this->item->id);
				}
				if ($this->canDo->get('placeholder.create'))
				{
					JToolBarHelper::custom('placeholder.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('placeholder.cancel', 'JTOOLBAR_CLOSE');
			}
		}
